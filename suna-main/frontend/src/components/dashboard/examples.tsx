...
{
    title: '정책 효과 분석',
    query: `분석할 정책명과 국가(또는 그룹)을 입력하세요.
    [입력 형식] 
    정책명 | 국가목록(or 그룹) [| 기간]

    [예시]
    - "코로나19 경제정책 | 주요국
    - "생물다양성 보전정책 | 코스타리카, 에콰도르, 마다가스카르"
    - "친환경 교통정책 | 노르웨이, 네덜란드, 덴마크"
    - "디지털 교육격차 | 주요국 | 코로나19 이후

    ※ 자연어 입력도 허용됩니다.
  예: "주요국의 코로나19 대응 경제정책과 경제회복 속도를 비교분석해줘"
     → 자동으로 '코로나19 경제정책 | 주요국 로 변환
  `,
    hiddenPrompt: `
# 정책 효과 분석
...
1) 정형 입력: '정책명 | 국가목록(or 그룹) [| 기간]'  
2) 자연어 입력: "코로나19 경제정책 효과비교", "생물다양성 보전 정책효과 분석" 등

---

## 자연어 입력 파싱 규칙
- "효과비교", "효과분석", "성과분석", "보급현황 분석", "대응정책 분석" → 전부 '정책효과' 모듈로 매핑  
- 단, 보고서 작성 시 입력 키워드에 맞춰 강조 (예: "효과비교" → 비교 강조, "보급현황" → 보급률 중심 등)  
- "정책효과", "정책분석", "정책평가" → 정책효과 모듈  
- "전망", "시나리오", "예측" → 전망 모듈 (이 경우는 정책효과 분석이 아님)

---
## 절차
0) 파라미터 파싱  
1) 내부 문서 검색 (정책 정의, 성과 지표)  
2) (필요시) 웹 보완 검색 - OECD, World Bank, UN 공식 자료 우선  
3) 정책 효과 분석 - 정책수단 vs 성과지표 매핑, 국가별 비교  
4) 요약 리포트 생성 - 핵심 요약, 국가별 사례, 정책 효과, 시사점  
5) 품질 점검 - 공식 출처만 사용, 효과 단정 대신 "제한적 증거" 표현  



## 0) 파라미터 파싱
- policy: 정책명 (코로나19 경제정책, 생물다양성 보전정책 등)
- countries: 국가/그룹
- period: 분석 기간
- module: 정책효과 (강제 적용)


## 1) 내부 문서 조회 (첫 번째 도구 호출, 필수)
🛑 **규칙**: 이 단계에서 web_search를 사용하지 마세요.

## 1) 내부 문서 조회
- 도구: 'search_internal_documents'
- 확보 내용: 정책 정의, 지표/성과 측정 방법, 관련 연구 요약
- 보고: 
...
- 내부/외부 출처 구분
- 지표 단위/정의 명확화
- 정책 효과를 단정하지 않고 “제한적 증거/추세 확인” 수준으로 표현
...
## 2) 국가별 사례 비교
- 국가별 주요 정책 수단, 지표, 성과 비교 및 요약

## 3) 정책 효과 분석
- 공통 성공요인, 제약요인, 차별화 요소

## 4) 시사점
- 우리나라/기관에 적용 가능한 교훈 및 권고사항
