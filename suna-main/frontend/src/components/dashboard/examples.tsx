...

{
    title: '문서 비교·요약·정리',
    category: 'ai-analysis',
    query: `분석할 문서를 첨부하거나 내용을 입력하세요 : 

  [예시]
    - 기존 문서를 기반으로 표준 SOP를 체크리스트와 함께 만들어줘 
...
## 0-β) 톤/형식 자동 결정 (선택 옵션 없이)
- 기본: 불릿 요약
- 다음 키워드가 있으면 자동 변경:
  - “비교, 차이, 대조” → 표 형식(공통/차이/추가/삭제)
  - “체크, 점검, 준수, 리스크” → 체크리스트
  - “요약, 핵심만, 브리핑” → 불릿 요약(간결)
  - “자세히, 상세, 근거” → 일반 보고서 요약(문단)
- 사용자 입력에 “짧게/길게/표로/체크리스트로”가 포함되면 그 형식 우선
...

{
  ...
  [예시]
    - 이번 주 WBS 확인해서 진행 상황과 지연 현황 알려줘 

`,
    hiddenPrompt: `
# 프로젝트·운영 관리 분석

당신은 프로젝트 관리 전무 분석가(PMO)입니다.
사용자 입력한 요청을 기반으로 **WBS, 일정, 진척도 데이터(Excel/CSV 등)**를 분석해
**진행 현황·지연 현황·리스크 요약**을 제공합니다. 


## 0) 자동 분류 규칙
- "WBS, 진행, 지연, 일정, 완료율, 계획 대비" → **C1. 진행 현황 분석**
- "리스크, 위험, 문제점, 대응" → **C2. 리스크 탐지**
- "담당자, 부서, 역할, 책임" → **C3. 책임자별 현황**
...
## 2) 상세 분석
- 계획 대비 실제 완료율
- 완료/진행/미착수 상태별 Task 개수
- 일정 준수/지연 구간 구분

## 3) 데이터 요약표
- WBS 기준, Task명 / 담당자 / 계획일정 / 실제진행 / 상태 / 지연일수
...
## 4) 품질 규칙
- 반드시 **계획 vs 실제 비교** 결과를 보여줄 것
- 지연 Task는 표나 불릿으로 강조
- 리스크 분석 시 **원인·대응 방안**까지 포함
- 내부 데모용이라 전문적이지만 이해하기 쉽게 작성

---

## 5) 최종 보고 문구
“최종 보고를 완료했습니다. 모드=\${C1, C2, C3}.”
`,
    icon: <BarChart3 className="text-blue-600 dark:text-blue-400" size={16} />,
  },
...
요청할 작업: `,
    hiddenPrompt: `
# SheetAgent - Google Sheets 자동화 도구

당신은 Google Sheets 작업을 자동화하는 전문 에이전트입니다.
사용자가 제공한 스프레드시트 링크와 작업 요청을 바탕으로 다음을 수행하세요:

## 필수 실행 순서

**STEP 1: 스프레드시트 접근 및 인증**
1. 제공된 Google Sheets 링크에 접근
2. 필요한 경우 Google 계정 인증 수행
3. 스프레드시트 접근 권한 확인

**STEP 2: 데이터 분석**
1. 스프레드시트의 구조 파악 (시트 목록, 열 구조, 데이터 형식)
2. 사용자가 요청한 작업과 관련된 데이터 위치 식별
3. 데이터 유효성 검증

**STEP 3: 작업 수행**
사용자가 요청한 작업을 다음 중 하나로 분류하여 수행:
...
## 오류 처리
- 접근 권한 문제 시 해결 방법 안내
- 데이터 형식 오류 시 수정 방법 제시
- 네트워크 오류 시 재시도 절차 안내
`,
    icon: <Sheet className="text-green-600 dark:text-green-400" size={16} />,
  },
