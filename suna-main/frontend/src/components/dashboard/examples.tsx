...
`,
  ...
  {
    title: '민원 대응 전략 수립',
    query: `민원 유형 또는 이슈 키워드(예: 교통 정체, 소음 문제, 주차 부족 등) : 
지역명(예: 서울 강남구, 부산 해운대구 등) : 
예상 민원자 연령대(예: 20-30대, 40-50대, 전 연령대 등) : `,
## 민원 대응 전략 수립 자동화 가이드

1. 웹 크롤링 단계:
   - 지역 언론 뉴스 사이트에서 관련 이슈 검색
   - 주요 포털 뉴스, 지역신문, SNS 트렌드 분석
   - 국민신문고 민원 현황 및 유사 사례 조사
   - 최근 3개월 내 관련 민원 동향 파악

2. 내부 문서 활용:
   - 과거 동일/유사 민원 처리 사례집 검토
   - 기존 정책 가이드라인 및 FAQ 문서 참조
   - 부서별 대응 매뉴얼 및 표준 답변안 수집
   - 관련 법규 및 조례 조항 확인

3. 민원 분석 및 전략 수립:
   - 민원의 핵심 이슈 및 시민 불편사항 분석
   - 연령대별, 지역별 민원 패턴 파악
   - 정책적 해결방안과 단기/장기 대응책 구분
   - 예상 질의사항 및 표준 답변안 도출

4. 결과물 생성:
   - 현재 이슈 요약 및 민원 발생 배경 분석
   - 시민 예상 질문 리스트 및 답변 초안 작성
   - 부서별 역할 분담 및 대응 프로세스 제안
   - 향후 유사 민원 예방 방안 및 정책 개선안 제시

5. 최종 검토:
   - 법적 검토사항 및 리스크 요소 점검
   - 타 지자체 유사사례 벤치마킹 결과 포함
   - 시민 소통 채널별 맞춤형 대응방안 구성
`,
