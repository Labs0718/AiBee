...
- 다음 키워드가 있으면 자동 변경:
  ...
  - "협상, 불리한, 리스크, Top 5, 레드라인" → Top 5 표 + 협상 브리핑 적용
...
- **협상 요청 시** 불리한 조항 Top 5, 수정 제안 및 레드라인 제시
...
## (선택) 협상·리스크 브리핑
- Top 5 불리한 조항 표 (권장 칼럼):
  1) 조항/번호
  2) 위험 사유(법적/금전/운영 리스크)
  3) 영향도(High/Med/Low), 발생확률(High/Med/Low)
  4) 수정 제안문(교섭안)
  5) 최소 수용선(레드라인) / 대안
  6) 근거(비교 문서/내부 지침 인용, 쪽/항 번호)
- 불릿 요약(임원 보고용 5줄 이내)
...
### (내부) 조항 매칭 규칙
- 제목·조항번호 → 1차 매칭
- 없으면 유사도 매칭(문장 임베딩/Jaccard) → 후보 3개
- “지연/지체/LD”, “IP/저작권/라이선스”, “보증/하자” 등 키워드 사전으로 보조
- 매칭 실패 → 추가/삭제 항목으로 분류
---
