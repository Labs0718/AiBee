...
#### 회의명 자동 생성 로직:
**1순위: 명시적 회의명 추출**
- "오늘 [회의명] 회의를...", "[팀명] 정기회의", "[프로젝트명] 미팅" 등 직접 언급된 회의명
- 회의 시작 부분에서 "안녕하세요, [회의명]에 참석해주셔서..." 형태

**2순위: 주요 안건 기반 회의명 생성**
- 가장 중요한 안건을 기반으로 회의명 자동 생성
- 예: "신제품 개발 현황" → "신제품 개발팀 정기 회의"
- 예: "예산 검토, 마케팅 전략" → "사업 기획 회의"

**3순위: 참석자 기반 회의명 추정**
- 참석자 직책/팀명으로 회의 성격 추정
- 예: "팀장, 과장, 대리" → "[부서명] 팀 회의"
- 예: "CEO, 임원진" → "경영진 회의"

**4순위: 기본 회의명 적용**
- 위 모든 방법으로 추출 불가시: "정기 회의"
...
#### STEP 2: 기존 템플릿 구조에 맞춘 내용 분석 및 분류
**회의명 우선 분석:**
...
**새로운 템플릿에 맞춘 데이터 입력 영역**:
- B5:D5 (회의명) → 회의 제목/명칭 입력 (C5, D5도 필요시 활용)
- B6 (일시) → 회의 일시 데이터 입력
- D6 (장소) → 회의 장소 데이터 입력
- B7 (작성일) → 작성일 데이터 입력
- D7 (작성자) → 작성자명 데이터 입력
- B8:D8 (참석자) → 참석자 목록 데이터 입력 
- B9:D9 (회의안건) → 안건 내용 데이터 입력 
- B10:D10 (요약내용) → 회의 요약 내용 데이터 입력 
- B11:D11 (결정사항) → 결정사항 내용 데이터 입력 
- B12:D12 (특이사항) → 특이사항 내용 데이터 입력 
