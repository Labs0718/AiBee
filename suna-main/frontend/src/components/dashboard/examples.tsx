...
{
    title: '시계열 추이·예측 분석',
    query: `분석할 지표와 조건을 입력하세요. 
자연어로 작성해도 됩니다. 

[예시] 
- "출산율 | OECD | 1994-2024 | 2035까지 예측 | 모듈:정책효과" 
- "선진국 출산율 변화 및 정책효과 분석 보고서" 
- "G20 도시화 전망" 
...
사용자 입력은 두 가지 형식을 모두 허용합니다:
1) 정형 입력: '지표 | 국가목록(or 그룹) | 과거시작연도-과거끝연도 | 예측끝연도 [| 모듈:정책효과/전망]'
2) 자연어 입력: "OECD 회원국의 출산율 변화를 최근 30년간 추적하고 각국의 출산장려정책 효과를 비교분석한 보고서를 생성해줘"

---
## 입력 파싱 규칙

- **지표 추출**: "출산율", "도시화율", "GDP 성장률", "청년실업률", "탄소배출량" 등 → indicator  
- **국가/그룹 추출**: "OECD", "G20", "주요 20개국", "선진국" 등 → countries  
- **기간 추출**: "최근 30년", "1970년부터", "최근 20년" 등 → period  
- **예측 범위 추출**: "예측", "전망", "2035까지" 등 → forecast_to  
- **모듈 자동 해석**:  
  - 사용자가 '모듈:'을 명시 → 해당 모듈 적용  
  - 미지정 시 규칙:  
    - 문구에 '정책/효과/분석/평가' 포함 → 모듈=정책효과  
    - 문구에 '전망/예측/시나리오/목표' 포함 → 모듈=전망  
    - 둘 다 없으면 모듈 없음 (→ 기본 리포트만 제공: 핵심 요약 + 해석 및 시사점)

**자동 변환 예시**
- "선진국 출산율 변화 및 정책효과 분석 보고서"  
   → '출산율 | OECD | 최근30년 | 2035까지 예측 | 모듈:정책효과'  
- "G20 도시화 전망"  
