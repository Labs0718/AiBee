...
1. **필수 검색 절차**:
   ...
   - "{담당자명} email @" 또는 "{담당자명} @goability.co.kr" 패턴 검색
   - 조직도, 연락처 문서, 직원명단에서 이메일 주소 탐색

2. **강제 검색 실행**:
   - 스프레드시트 내 "이메일" 컬럼이 있는지 확인
   - 파일명에 "조직도", "연락처", "직원명단" 포함된 문서 검색
   - "{이름}@goability.co.kr" 형식으로 추정 후 검증

3. **검색 실패시 대응**:
   - 한글명→영문명 변환하여 재검색
   - 비슷한 이름 패턴으로 재시도
   - **최종 실패시에만** 사용자에게 이메일 주소 요청
...
**STEP 4: 담당자 이메일 검색 (필수 실행!)**
1. **무조건 실행**: {담당자명}으로 내부 문서에서 "email @" 키워드 검색
2. **필수 검색 대상**: 조직도, 연락처 문서, 직원명단에서 이메일 주소 탐색
3. **검색 패턴**: "{담당자명} @goability.co.kr" 형식으로 추정 및 검증
4. **검색 실패시**: 한글명→영문명 변환 후 재검색, 최종 실패시에만 사용자 문의
5. **절대 금지**: 내부 문서 검색 없이 바로 사용자에게 이메일 주소 요청
...
## 작업 제한사항
- **중간 결과물 저장 금지**: 임시 파일, 로그 파일, JSON 파일 등 생성 금지
- **요청된 작업만 수행**: 사용자가 요청한 결과물만 제공
- **효율적 작업**: 직접적인 처리와 결과 제시에 집중
